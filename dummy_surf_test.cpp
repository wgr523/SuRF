...
#include "dynamic_include/dummy_surf.hpp"

using namespace surf;
using namespace std::chrono;

int LookUpKeyTest(DummySurf* surf, const std::string& key, bool ans){
...
int LookUpRangeTest(DummySurf* surf, const std::string& left_key, const bool left_inclusive, 
...
int main() {
    ...
    DummySurf* surf = new DummySurf(keys,500,&mainMemory);
    ...
    std::cout<< "random 2000 point lookup " << duration << " s " << "FP rate " << FP/2000 <<"%" <<std::endl;
    ...
    while(i < 1000) {
       ...
       FP += LookUpRangeTest(surf,dataSet[RandIndex],true,"zzz",true,true);
       i++;
    }
    while(i < 2000) {
       int RandIndex = rand() % dataSetRef.size();
       FP += LookUpRangeTest(surf,dataSetRef[RandIndex],true,"zzz",true,false);
       i++;
    }
    ...
    std::cout<< "random 2000 range lookup " << duration << " s " << "FP rate " << FP/2000 << "%" <<std::endl;
